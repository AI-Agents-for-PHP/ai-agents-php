...
namespace CaiqueBispo\AiAgentsPhp\AgentTraits;
...
trait WeatherTrait {
    ...
    public function getTodayWeather(float $lat, float $long) : array {
        $lat = urlencode($lat);
        $long = urlencode($long);
        $url = "https://api.openweathermap.org/data/2.5/weather?units=imperial&lat=" . $lat . "&lon=" . $long . "&appid=" . $_ENV['OPENWEATHER_API_KEY'];
        $ch = curl_init();
